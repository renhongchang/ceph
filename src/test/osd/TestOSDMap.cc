...
TEST_F(OSDMapTest, RemovesRedundantTemps) {
  ...
  OSDMap::remove_redundant_temporaries(g_ceph_context, osdmap, &pgtemp_map);

  EXPECT_EQ((unsigned)0, pgtemp_map.new_pg_temp[pgid].size());
  EXPECT_EQ(-1, pgtemp_map.new_primary_temp[pgid]);
}
...
TEST_F(OSDMapTest, KeepsNecessaryTemps) {
  ...
  OSDMap::remove_redundant_temporaries(g_ceph_context, osdmap, &pgtemp_map);
  EXPECT_EQ(pgtemp_map.new_pg_temp[pgid], up_osds);
  EXPECT_EQ(pgtemp_map.new_primary_temp[pgid], up_primary);
}
